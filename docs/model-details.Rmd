...
title: Model of COVID-19 transmission in the U.S.A. by state
---
...
forecast_date <- Sys.getenv("fdt", unset = "2021-05-17")
forecast_locs <- c("06") 
...
Prior to the availability of vaccination, the primary interventions in all states was the adoption of social distancing behaviors and improved hygiene.
...
mob <- load_google_mobility("2021-05-19", rootdir = "..") %>%
...
Vaccination is now available and has proven critical to the control of the pandemic. 
We use vaccination time series in [this repository](https://www.github.com/govex/COVID-19) created by Johns Hopkins University which was obtained from state and CDC dashboards.

```{r vaccination, eval = TRUE, fig.height=3}
...
vacc_ts <- read_csv(
  ...
  filter(FIPS %in% forecast_locs & Vaccine_Type == "All") %>%
...
vacc_ts2 <- vacc_ts %>% mutate(doses = zoo::na.approx(doses, rule = 2))
vacc_ts2 %>%
  ggplot(aes(x = target_end_date, y = doses)) +
  geom_line() +
  labs(y = "Doses adminsistered", x = "Date")
...
\frac{dZ_r(t)}{dt} =& \gamma_{z} Z\left( t \right) \\
...
\frac{dD_r(t)}{dt} =& \gamma_{d} D\left( t \right)
\end{align*}

This diagram provides a visualization of these equations:

```{r ode-diagram}
knitr::include_graphics("ode-diagram.png")
```
The three compartments at the right side of the diagram enclosed by a grey background correspond to our observable variables: case notifications, hospital admissions, and death notifications.
...
* Vaccination, which is assumed to reduce the average susceptibility of the population in proportion to the number of doses administered.
