...
all_data <- state_summaries %>%
  ...
  # mutate(variable = ifelse(variable == "actual_daily_cases", "cases", "deaths"))
plot_data <- function(database, state, legend = FALSE) {
  
  statedata <- database %>% filter(location == state)
  ...
  p_data <- plotly::plot_ly(data = statedata, x = ~date, showlegend = legend) %>% 
    plotly::add_trace(y = ~actual_daily_cases, type = 'bar',
                      name = 'New case notifications',
                      marker = list(color = col.cases),
                      legendgroup = 'group1',
                      hoverinfo = "x+text",
                      hoverlabel = hoverlabel,
                      text = ~paste(display(actual_daily_cases),serif("new case reports")) ) %>% 
    plotly::add_trace(y = ~actual_daily_deaths, 
                      name = 'New death notifications', type = 'scatter', mode = 'lines',
                      line = list(color = col.deaths, width = data.lwd, shape = 'hvh'),
                      legendgroup = 'group2',
                      hoverinfo = "x+text",
                      hoverlabel = hoverlabel,
                      text = ~paste(display(actual_daily_deaths),serif("new deaths")) ) %>% 
    plotly::layout(yaxis = list(title = state))
  p_data
}
plots <- list()
plots[[1]] <- plot_data(all_data, "Georgia", legend = FALSE) 
plots[[2]] <- plot_data(all_data, "New York") 
plots[[3]] <- plot_data(all_data, "Washington") 
plots[[4]] <- plot_data(all_data, "Wyoming") 
dash <- plotly::subplot(plots, nrows = length(plots), shareX = TRUE, titleY = TRUE) %>%
  plotly::layout(
    xaxis = list(
      spikethickness = 1,
      spikedash = "dot",
