...
score <- function(path, tdf2) {
  fcst <-
    read_forecast(path) %>% mutate(target_type = str_remove(target, "^\\d+ "))
  ...
  probsumdf <- probdf %>% group_by(forecast_date, target) %>%
    summarise(
      mean_quantile_score = mean(pinball_loss),
      mean_brier_score = mean(brier_loss),
      .groups = "drop"
    )
...
summary_plot_path <-
  file.path("metrics", paste0(ddt, "-score-by-loc-type.csv"))
write_csv(summary$by_loc_type, path = summary_plot_path)

summary_plot_path <-
  file.path("metrics", paste0(ddt, "-score-by-loc-type-targ-type.csv"))
write_csv(summary$by_loc_type_targ_type, path = summary_plot_path)

summary_plot_path <-
  file.path("metrics",
            paste0(ddt, "-score-by-loc-type-targ-type-forecast-date.csv"))
